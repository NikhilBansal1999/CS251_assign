...
command = "gcc -pg -o executable "+prog_name
os.system(command)
    ...
    for line in analysis_file:
        ...
        if line_count > 6:
            ...
            if len(line_split) > 1:
                if line.startswith('---'):
                    parent = list()
                elif line_split[-2] == func[0] and ("/" not in line_split[-3]):
                    start_storing = 1
                    for par in parent:
                        for child in func_index:
                            if child[0] == par[0]:
                                ...
                                for output in output_data:
                                    if output[0] == func[0] and output[2] == child[0]:
                                        to_append = 0
                                if to_append:
